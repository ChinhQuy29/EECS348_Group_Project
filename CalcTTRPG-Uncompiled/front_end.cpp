...
Calculator::Calculator(QWidget *parent) : QWidget(parent) {
  ...
  QString buttons[7][5] = {
      ...
      {QString(u'π'), QString("d10"), QString("d12"), QString("d20"),
       QString("d100")},
      {QString('e'), QString('('), QString(')'), QString('%'), QString('^')},
      {QString(""), QString('7'), QString('8'), QString('9'), QString(u'÷')},
      {QString("History"), QString('4'), QString('5'), QString('6'),
       QString(u'×')},
      {QString("Delete"), QString('1'), QString('2'), QString('3'),
       QString('-')},
      {QString("Clear"), QString('0'), QString('.'), QString('='),
       QString('+')}};
  ...
  for (int r = 0; r < 7; r++) {
    for (int c = 0; c < 5; c++) {
      ...
      else if (text == QString("Clear") || text == QString("Delete") ||
               text == QString("History"))
      ...
      // Connections
      if (text[0].isDigit() || text == QString('.') || text == QString('+') ||
          text == QString('-') || text == QString(u'×') ||
          text == QString(u'÷') || text == QString('^') ||
          text == QString('%') || text == QString(u'π') ||
          text == QString('e') || text == QString('(') || text == QString(')'))
...
void Calculator::equalsClicked() {
  ...
  try {
    ...
    const QStringView before =
        QStringView(qExpression).first(e.getPosition() - 1);
    const QString middle = QString("<b style='color:red;'>%1</b>")
                               .arg(qExpression[e.getPosition() - 1]);
    const QStringView after = QStringView(qExpression).sliced(e.getPosition());
    const QStringView newExpression = before + middle + after;
    const QString emphasizedError = QString("%1 <i style='color:gray;'>%2</i>")
                                        .arg(newExpression)
                                        .arg(e.what());
...
void Calculator::clearClicked() { display->setText(""); }
...
History::History(QWidget *parent) : QWidget(parent) {
  ...
  QString buttons[7] = {QString(""),     QString(""),       QString(""),
                        QString(""),     QString("Return"), QString(""),
                        QString("Clear")};
...
void History::reloadHistory() {
  const std::vector<std::string> calculations =
      historyManager.getCalculations();
  historyDisplay->clear();
